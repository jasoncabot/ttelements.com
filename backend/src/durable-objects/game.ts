...
export const openSocketGameAction = (token: string) => `${durableObjectGameAction('open-ws')}&token=${token}`;

const updateGameDetailsAction = (gameId: string) => `${durableObjectGameAction('update-details')}&id=${gameId}`;
const listJoinGameAction = (gameId: string, creatorId: string, joinerId: string) =>
  `${durableObjectGameAction('list-join')}&id=${gameId}&creator_id=${creatorId}&joiner_id=${joinerId}`;
const listEndGameAction = (gameId: string, creatorId: string, joinerId: string) =>
...
export class Game implements DurableObject {
  connections: Map<SocketToken, Connection>;
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    switch (action) {
      ...
      case 'show': {
        ...
        const game = await this.state.storage.get<GameEntry>('entry');
        if (game) {
          return json(playerView(game, userId));
        } else {
          return missing({ error: 'game not found' });
        }
      }
      case 'list': {
        const userId = searchParams.get('userId')!;
        const gameRecords = await this.state.storage.list<JoinableGame>({ prefix: `u:${userId}:in-progress` });
        const games: JoinableGame[] = [];
        gameRecords.forEach((g) => {
          games.push(g);
        });
        return status(200, { games } as { games: JoinableGame[] });
      }
      case 'list-open': {
        const gameRecords = await this.state.storage.list<JoinableGame>({ prefix: 'open' });
        const games: JoinableGame[] = [];
        gameRecords.forEach((g) => {
          games.push(g);
        });
      ...
      case 'choose-cards': {
        ...
        try {
          const result = await handleChooseCards(this.state, this.env, this.connections.values(), req, userId);
          return json(result);
        } catch (e: any) {
          return status(400, { error: e.message });
        }
      ...
      case 'play-card': {
        ...
        try {
          const result = await handlePlayCard(this.state, this.env, this.connections.values(), req, userId);
          return json(result);
        } catch (e: any) {
          return status(400, { error: e.message });
        }
      ...
      case 'trade-cards': {
        ...
        try {
          const result = await handleTradeCards(this.state, this.env, this.connections.values(), req, userId);
          return json(result);
        } catch (e: any) {
          return status(400, { error: e.message });
        }
      ...
      case 'list-join': {
        ...
        try {
          const game = await this.state.storage.get<JoinableGame>(`u:${creatorId}:open:${gameId}`);

          this.state.storage.put(`u:${creatorId}:in-progress:${gameId}`, game);
          this.state.storage.put(`u:${joinerId}:in-progress:${gameId}`, game);
          this.state.storage.delete(`u:${creatorId}:open:${gameId}`);
          this.state.storage.delete(`open:${gameId}`);
          return status(200);
        } catch (e) {
          return status(400, { error: 'Game not found' });
        }
      }
      case 'list-game-over': {
        this.state.storage.delete(`u:${searchParams.get('creator_id')!}:in-progress:${searchParams.get('id')!}`);
        this.state.storage.delete(`u:${searchParams.get('joiner_id')!}:in-progress:${searchParams.get('id')!}`);
        return status(200);
      }
      case 'update-details': {
        const game: GameEntry = await request.json<GameEntry>();
        this.state.storage.put<GameEntry>('entry', game);
        return status(201, game);
      }
      case 'open-ws': {
        const token = searchParams.get('token');
        if (!token) {
          return status(400, { error: 'expected token' });
        }

        const socketToken = (await this.env.SOCKET_TOKENS.get(token, 'json')) as TokenEntry;
        if (!socketToken || !socketToken.gameId || !socketToken.userId) {
          return status(401, { error: 'unable to load' });
        }
        ...
        if (game.id !== socketToken.gameId) {
          return status(401, { error: 'unable to load game id' });
        }
  ...
  handleSession(socket: WebSocket, token: string, userId: string, userName: string, emailHash: string) {
    ...
    socket.addEventListener('message', async (msg) => {
      ...
      try {
        switch (command.type) {
          case 'choose-cards':
            await handleChooseCards(this.state, this.env, this.connections.values(), command.data as ChooseCardsRequest, userId);
            break;
          case 'play-card':
...
const playerView = (game: GameEntry, userId: string) => {
  ...
  const toSpaceResponse = (space: SpaceEntry, userId: string) => {
    if (!space.playerId || !space.card) return { owner: 'empty' } as SpaceResponse;
...
const onGameStateChange = (game: GameEntry, connections: IterableIterator<Connection>) => {
...
const handlePlayCard = async (state: DurableObjectState, env: Bindings, connections: IterableIterator<Connection>, req: PlayCardRequest, userId: string) => {
...
const handleChooseCards = async (
  ...
  const game = await state.storage.get<GameEntry>('entry');
  if (!game || !['PickInProgress', 'WaitingForOtherPlayer'].includes(game.state)) {
  ...
  game.players[playerIndex].cards = request.cards.map((card) => {
    ...
    const matchingCard = cardResponse.findIndex((c) => c.edition === card.edition && c.kind === card.kind);
    if (matchingCard < 0) {
...
const handleJoin = async (
  ...
  const id = env.GAME.idFromName('global');
  const obj = env.GAME.get(id);
  obj.fetch(listJoinGameAction(game.id, game.players[0].id, userId), { method: 'POST' });

  return playerView(game, userId);
};

const handleTradeCards = (state: DurableObjectState, env: Bindings, connections: IterableIterator<Connection>, request: TradeCardsRequest, userId: string) => {
...
const handleCreate = async (
  ...
  const game: GameEntry = {
    ...
    turn: 0,
    turnEndsAt: endsAt,
    board: Array(9).fill({ card: undefined, element: undefined, playerId: undefined })
  };
  ...
  const obj = env.GAME.get(id);
  obj.fetch(updateGameDetailsAction(gameId), { method: 'POST', body: JSON.stringify(game) });
